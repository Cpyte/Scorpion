...
/*
 * Flash access for the ESP32-S3 via the legacy SPI-flash driver in ROM.
 *
 * Addresses below are fixed by the S3 mask ROM and match Espressif's
 * generated linker fragment (components/esp_rom/esp32s3/ld/
 * esp32s3.rom.ld, group "spiflash_legacy"):
 *
 *   esp_rom_spiflash_erase_sector = 0x400009fc
 *   esp_rom_spiflash_write        = 0x40000a14
 *   esp_rom_spiflash_read         = 0x40000a20
 *
 ...
 * three functions execute from ROM while the kernel runs from RAM;
 * interrupts are masked around each operation anyway because erase/
 * program stall the CPU for tens of milliseconds.
 *
 * Alignment rules of the legacy ROM API (identical to the C3):
 *   - read:  address must be word-aligned; length is rounded up here
 *            and served from a bounce buffer.
 *   - write: address AND length must be multiples of 4.
 *   - erase: takes a SECTOR NUMBER (offset / 4096), not an address.
 */

#define ROM_SPIFLASH_ERASE_SECTOR 0x400009fcu
#define ROM_SPIFLASH_WRITE        0x40000a14u
#define ROM_SPIFLASH_READ         0x40000a20u
